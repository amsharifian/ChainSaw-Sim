...
{
  DEBUG_SLICC(MedPrio, ":225: ", m_machineID);
;
  DEBUG_SLICC(MedPrio, ":226: ", param_requestor);
;
  DEBUG_SLICC(MedPrio, ":227: ", param_addr);
;
    if (ASSERT_FLAG && !(((map_L1CacheMachId_to_L2Cache(param_addr, param_requestor)) == m_machineID))) {
    cerr << "Runtime Error at :228, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
