...
bool Sequencer::tryL1CacheAccess(const Address & addr)
{
    if (Protocol::m_TwoLevelCache) {
      return m_chip_ptr->m_L1Cache_L1DcacheMemory_vec[m_version]->tryAccCacheAccess(line_address(addr));
    }
    else {
      return m_chip_ptr->m_L1Cache_cacheMemory_vec[m_version]->tryAccCacheAccess(line_address(addr));
    }
}


bool Sequencer::tryAccCacheAccess(const Address& addr)
{
    Chip *ptr = dynamic_cast<Chip*>(m_chip_ptr);
    MachineID l2_mach = map_L2ChipId_to_L2Cache(addr, m_chip_ptr->getID() );
    int l2_ver = l2_mach.num%RubyConfig::numberOfL2CachePerChip();
    return ptr->m_L2Cache_L2cacheMemory_vec[l2_ver]->tryAccCacheAccess(line_address(addr));
}
