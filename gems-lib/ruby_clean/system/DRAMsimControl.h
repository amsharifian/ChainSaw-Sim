 ...
 * DRAMsimControl.h
 * 
 * Description:  See DRAMsimControl.C
 *
 * $Id: $
 *
 */

#ifndef DRAMsim_CONTROL_H
#define DRAMsim_CONTROL_H
...
// the comments in header to DRAMsimControl.C
...
class DRAMsimControl;
class some_object
{
  public: 
    DRAMsimControl* Controller;
    void read_complete(unsigned, uint64_t, uint64_t);
    void write_complete(unsigned, uint64_t, uint64_t);
    some_object(DRAMsimControl* Control) {Controller = Control;}

};



class DRAMsimControl : public Consumer, public AbstractMemOrCache {
public:

  // Constructors
  DRAMsimControl (AbstractChip* chip_ptr, int version);
  ...
  // Callback from DRAMsim->MemControl 
  void enqueueToResponse (physical_address_t addr);
...
private:
  ...
  DRAMsimControl (const DRAMsimControl& obj);
  DRAMsimControl& operator=(const DRAMsimControl& obj);
  ...
  some_object* obj;
  Map<Address, MemoryNode>* m_active_queue; 
