...
public:
  ...
  // Accelerator performs a cache access and see if we hit or not.  Return true on a hit.
  bool tryAccCacheAccess(const Address& address);
  ...
  // ska -- unset the lock bit for the given line

  void decrementKeyCountForBlock(const Address& addr);
  void setKeyCountForBlock(const Address& addr, unsigned count);
...

{
  ...
  // Pseudo LRU not support. If we want to support we need to template it just
  // like LRUPolicy.h

  m_replacementPolicy_ptr = new LRUPolicy<ENTRY>(m_cache_num_sets, m_cache_assoc, &m_cache);
...
template<class ENTRY>
inline
bool CacheMemory<ENTRY>::tryAccCacheAccess(const Address& address)
{
  assert(address == line_address(address));
  DEBUG_EXPR(CACHE_COMP, HighPrio, address);
  Index cacheSet = addressToCacheSet(address);
  ...
  { // Do we even have a tag match?
    ...
    if(entry.m_Permission == AccessPermission_Read_Only ||
       ...
       entry.m_Permission == AccessPermission_Stale )
    {
            return true;
    }
    return false;
