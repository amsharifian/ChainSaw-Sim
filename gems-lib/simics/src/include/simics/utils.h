...
/* Macros to create a sub-pointer to an integer of possibly smaller size */
#ifdef HOST_BIG_ENDIAN
#define SUB_TYPE_PTR(lval, type)                                \
    (sizeof(type) < sizeof(lval)                                \
     ? (type *)((char *)&(lval) + sizeof(lval) - sizeof(type))  \
     : (type *)&(lval))
#define SUB_TYPE_PTR_SZ(lval, size)                     \
    ((void *)((char *)&(lval) + sizeof(lval) - (size)))
#else
/* note the intermediate (void *) cast, to avoid illegal aliasing troubles */
#define SUB_TYPE_PTR(lval, type) ((type *)(void *)&(lval))
#define SUB_TYPE_PTR_SZ(lval, size) ((void *)&(lval))
#endif
